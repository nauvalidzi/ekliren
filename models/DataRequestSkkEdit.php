 ...
 * Page class
 */
class DataRequestSkkEdit extends DataRequestSkk
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DataRequestSkkEdit";
    ...
    protected function validateForm()
    {
        ...
        if ($this->id_request->Required) {
            if (!$this->id_request->IsDetailKey && EmptyValue($this->id_request->FormValue)) {
                $this->id_request->addErrorMessage(str_replace("%s", $this->id_request->caption(), $this->id_request->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // nrp
            $this->nrp->setDbValueDef($rsnew, $this->nrp->CurrentValue, "", $this->nrp->ReadOnly);
            ...
            // pangkat
            $this->pangkat->setDbValueDef($rsnew, $this->pangkat->CurrentValue, 0, $this->pangkat->ReadOnly);

            // jabatan
            $this->jabatan->setDbValueDef($rsnew, $this->jabatan->CurrentValue, 0, $this->jabatan->ReadOnly);

            // keperluan
            $this->keperluan->setDbValueDef($rsnew, $this->keperluan->CurrentValue, 0, $this->keperluan->ReadOnly);

            // kategori_pemohon
            $this->kategori_pemohon->setDbValueDef($rsnew, $this->kategori_pemohon->CurrentValue, "", $this->kategori_pemohon->ReadOnly);

            // scan_lhkpn
            if ($this->scan_lhkpn->Visible && !$this->scan_lhkpn->ReadOnly && !$this->scan_lhkpn->Upload->KeepFile) {
                $this->scan_lhkpn->Upload->DbValue = $rsold['scan_lhkpn']; // Get original value
            ...
            // scan_lhkasn
            if ($this->scan_lhkasn->Visible && !$this->scan_lhkasn->ReadOnly && !$this->scan_lhkasn->Upload->KeepFile) {
                $this->scan_lhkasn->Upload->DbValue = $rsold['scan_lhkasn']; // Get original value
            ...
            // email_pemohon
            $this->email_pemohon->setDbValueDef($rsnew, $this->email_pemohon->CurrentValue, null, $this->email_pemohon->ReadOnly);

            // hukuman_disiplin
            $this->hukuman_disiplin->setDbValueDef($rsnew, $this->hukuman_disiplin->CurrentValue, null, $this->hukuman_disiplin->ReadOnly);
            ...
            if ($this->scan_lhkpn->Visible && !$this->scan_lhkpn->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->scan_lhkpn->Upload->FileName)) {
                    ...
                    $this->scan_lhkpn->setDbValueDef($rsnew, $this->scan_lhkpn->Upload->FileName, null, $this->scan_lhkpn->ReadOnly);
                }
            }
            if ($this->scan_lhkasn->Visible && !$this->scan_lhkasn->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->scan_lhkasn->Upload->FileName)) {
                    ...
                    $this->scan_lhkasn->setDbValueDef($rsnew, $this->scan_lhkasn->Upload->FileName, null, $this->scan_lhkasn->ReadOnly);
                }
            }
