 ...
 * Page class
 */
class SidangKodePerilakuGrid extends SidangKodePerilaku
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SidangKodePerilakuGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fsidang_kode_perilakugrid";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "SidangKodePerilakuAdd";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "v_sekretariat") {
            $masterTbl = Container("v_sekretariat");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("VSekretariatList"); // Return to master page
                return;
            } else {
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_sidang_kode_perilaku_jaksa") && $CurrentForm->hasValue("o_sidang_kode_perilaku_jaksa") && $this->sidang_kode_perilaku_jaksa->CurrentValue != $this->sidang_kode_perilaku_jaksa->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tempat_sidang_kode_perilaku") && $CurrentForm->hasValue("o_tempat_sidang_kode_perilaku") && $this->tempat_sidang_kode_perilaku->CurrentValue != $this->tempat_sidang_kode_perilaku->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_hukuman_administratif") && $CurrentForm->hasValue("o_hukuman_administratif") && $this->hukuman_administratif->CurrentValue != $this->hukuman_administratif->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_sk_nomor_kode_perilaku") && $CurrentForm->hasValue("o_sk_nomor_kode_perilaku") && $this->sk_nomor_kode_perilaku->CurrentValue != $this->sk_nomor_kode_perilaku->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tgl_sk_kode_perilaku") && $CurrentForm->hasValue("o_tgl_sk_kode_perilaku") && $this->tgl_sk_kode_perilaku->CurrentValue != $this->tgl_sk_kode_perilaku->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_status_hukuman_kode_perilaku") && $CurrentForm->hasValue("o_status_hukuman_kode_perilaku") && $this->status_hukuman_kode_perilaku->CurrentValue != $this->status_hukuman_kode_perilaku->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->sidang_kode_perilaku_jaksa->clearErrorMessage();
        $this->tempat_sidang_kode_perilaku->clearErrorMessage();
        $this->hukuman_administratif->clearErrorMessage();
        $this->sk_nomor_kode_perilaku->clearErrorMessage();
        $this->tgl_sk_kode_perilaku->clearErrorMessage();
        $this->status_hukuman_kode_perilaku->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->sidang_kode_perilaku_jaksa->CurrentValue = 'Tidak';
        $this->sidang_kode_perilaku_jaksa->OldValue = $this->sidang_kode_perilaku_jaksa->CurrentValue;
        $this->tempat_sidang_kode_perilaku->CurrentValue = null;
        $this->tempat_sidang_kode_perilaku->OldValue = $this->tempat_sidang_kode_perilaku->CurrentValue;
        $this->hukuman_administratif->CurrentValue = null;
        $this->hukuman_administratif->OldValue = $this->hukuman_administratif->CurrentValue;
        $this->sk_nomor_kode_perilaku->CurrentValue = null;
        $this->sk_nomor_kode_perilaku->OldValue = $this->sk_nomor_kode_perilaku->CurrentValue;
        $this->tgl_sk_kode_perilaku->CurrentValue = null;
        $this->tgl_sk_kode_perilaku->OldValue = $this->tgl_sk_kode_perilaku->CurrentValue;
        $this->status_hukuman_kode_perilaku->CurrentValue = null;
        $this->status_hukuman_kode_perilaku->OldValue = $this->status_hukuman_kode_perilaku->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_sidang_kode_perilaku_jaksa")) {
            $this->sidang_kode_perilaku_jaksa->setOldValue($CurrentForm->getValue("o_sidang_kode_perilaku_jaksa"));
        }
        ...
        if ($CurrentForm->hasValue("o_tempat_sidang_kode_perilaku")) {
            $this->tempat_sidang_kode_perilaku->setOldValue($CurrentForm->getValue("o_tempat_sidang_kode_perilaku"));
        }
        ...
        if ($CurrentForm->hasValue("o_hukuman_administratif")) {
            $this->hukuman_administratif->setOldValue($CurrentForm->getValue("o_hukuman_administratif"));
        }
        ...
        if ($CurrentForm->hasValue("o_sk_nomor_kode_perilaku")) {
            $this->sk_nomor_kode_perilaku->setOldValue($CurrentForm->getValue("o_sk_nomor_kode_perilaku"));
        }
        ...
        if ($CurrentForm->hasValue("o_tgl_sk_kode_perilaku")) {
            $this->tgl_sk_kode_perilaku->setOldValue($CurrentForm->getValue("o_tgl_sk_kode_perilaku"));
        }
        ...
        if ($CurrentForm->hasValue("o_status_hukuman_kode_perilaku")) {
            $this->status_hukuman_kode_perilaku->setOldValue($CurrentForm->getValue("o_status_hukuman_kode_perilaku"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['sidang_kode_perilaku_jaksa'] = $this->sidang_kode_perilaku_jaksa->CurrentValue;
        $row['tempat_sidang_kode_perilaku'] = $this->tempat_sidang_kode_perilaku->CurrentValue;
        $row['hukuman_administratif'] = $this->hukuman_administratif->CurrentValue;
        $row['sk_nomor_kode_perilaku'] = $this->sk_nomor_kode_perilaku->CurrentValue;
        $row['tgl_sk_kode_perilaku'] = $this->tgl_sk_kode_perilaku->CurrentValue;
        $row['status_hukuman_kode_perilaku'] = $this->status_hukuman_kode_perilaku->CurrentValue;
        return $row;
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "v_sekretariat") {
            $this->pid_request_skk->CurrentValue = $this->pid_request_skk->getSessionValue();
        }
        $conn = $this->getConnection();
        ...
        // sidang_kode_perilaku_jaksa
        $this->sidang_kode_perilaku_jaksa->setDbValueDef($rsnew, $this->sidang_kode_perilaku_jaksa->CurrentValue, null, false);

        // tempat_sidang_kode_perilaku
        $this->tempat_sidang_kode_perilaku->setDbValueDef($rsnew, $this->tempat_sidang_kode_perilaku->CurrentValue, null, false);

        // hukuman_administratif
        $this->hukuman_administratif->setDbValueDef($rsnew, $this->hukuman_administratif->CurrentValue, null, false);

        // sk_nomor_kode_perilaku
        $this->sk_nomor_kode_perilaku->setDbValueDef($rsnew, $this->sk_nomor_kode_perilaku->CurrentValue, null, false);

        // tgl_sk_kode_perilaku
        $this->tgl_sk_kode_perilaku->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_sk_kode_perilaku->CurrentValue, 0), null, false);

        // status_hukuman_kode_perilaku
        $this->status_hukuman_kode_perilaku->setDbValueDef($rsnew, $this->status_hukuman_kode_perilaku->CurrentValue, null, false);

        // pid_request_skk
        if ($this->pid_request_skk->getSessionValue() != "") {
            $rsnew['pid_request_skk'] = $this->pid_request_skk->getSessionValue();
        }
