 ...
 * Page class
 */
class SidangKodePerilakuList extends SidangKodePerilaku
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SidangKodePerilakuList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fsidang_kode_perilakulist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "SidangKodePerilakuDelete";
        $this->MultiUpdateUrl = "SidangKodePerilakuUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fsidang_kode_perilakulistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->pid_request_skk->AdvancedSearch->toJson(), ","); // Field pid_request_skk
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fsidang_kode_perilakulistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field pid_request_skk
        $this->pid_request_skk->AdvancedSearch->SearchValue = @$filter["x_pid_request_skk"];
        $this->pid_request_skk->AdvancedSearch->SearchOperator = @$filter["z_pid_request_skk"];
        $this->pid_request_skk->AdvancedSearch->SearchCondition = @$filter["v_pid_request_skk"];
        $this->pid_request_skk->AdvancedSearch->SearchValue2 = @$filter["y_pid_request_skk"];
        $this->pid_request_skk->AdvancedSearch->SearchOperator2 = @$filter["w_pid_request_skk"];
        $this->pid_request_skk->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->sidang_kode_perilaku_jaksa); // sidang_kode_perilaku_jaksa
            $this->updateSort($this->tempat_sidang_kode_perilaku); // tempat_sidang_kode_perilaku
            $this->updateSort($this->hukuman_administratif); // hukuman_administratif
            $this->updateSort($this->sk_nomor_kode_perilaku); // sk_nomor_kode_perilaku
            $this->updateSort($this->tgl_sk_kode_perilaku); // tgl_sk_kode_perilaku
            $this->updateSort($this->status_hukuman_kode_perilaku); // status_hukuman_kode_perilaku
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsidang_kode_perilakulistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsidang_kode_perilakulistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fsidang_kode_perilakulist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fsidang_kode_perilakulistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
