 ...
 * Page class
 */
class InspeksiGrid extends Inspeksi
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'inspeksi';

    // Page object name
    public $PageObjName = "InspeksiGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "finspeksigrid";
    ...
    public function __construct()
    {
        ...
        // Table object (inspeksi)
        if (!isset($GLOBALS["inspeksi"]) || get_class($GLOBALS["inspeksi"]) == PROJECT_NAMESPACE . "inspeksi") {
            $GLOBALS["inspeksi"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "InspeksiAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'inspeksi');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("inspeksi"));
                $doc->Text = @$content;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_inspeksi_kasus") && $CurrentForm->hasValue("o_inspeksi_kasus") && $this->inspeksi_kasus->CurrentValue != $this->inspeksi_kasus->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_pelanggaran_disiplin") && $CurrentForm->hasValue("o_pelanggaran_disiplin") && $this->pelanggaran_disiplin->CurrentValue != $this->pelanggaran_disiplin->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->inspeksi_kasus->clearErrorMessage();
        $this->pelanggaran_disiplin->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->pid_request_skk->CurrentValue = 0;
        $this->pid_request_skk->OldValue = $this->pid_request_skk->CurrentValue;
        $this->inspeksi_kasus->CurrentValue = 'Tidak';
        $this->inspeksi_kasus->OldValue = $this->inspeksi_kasus->CurrentValue;
        $this->pelanggaran_disiplin->CurrentValue = null;
        $this->pelanggaran_disiplin->OldValue = $this->pelanggaran_disiplin->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_inspeksi_kasus")) {
            $this->inspeksi_kasus->setOldValue($CurrentForm->getValue("o_inspeksi_kasus"));
        }
        ...
        if ($CurrentForm->hasValue("o_pelanggaran_disiplin")) {
            $this->pelanggaran_disiplin->setOldValue($CurrentForm->getValue("o_pelanggaran_disiplin"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['inspeksi_kasus'] = $this->inspeksi_kasus->CurrentValue;
        $row['pelanggaran_disiplin'] = $this->pelanggaran_disiplin->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->inspeksi_kasus->EditValue = $this->inspeksi_kasus->options(false);
            $this->inspeksi_kasus->PlaceHolder = RemoveHtml($this->inspeksi_kasus->caption());

            // pelanggaran_disiplin
            $this->pelanggaran_disiplin->EditAttrs["class"] = "form-control";
            $this->pelanggaran_disiplin->EditCustomAttributes = "";
            if (!$this->pelanggaran_disiplin->Raw) {
                $this->pelanggaran_disiplin->CurrentValue = HtmlDecode($this->pelanggaran_disiplin->CurrentValue);
            }
            $this->pelanggaran_disiplin->EditValue = HtmlEncode($this->pelanggaran_disiplin->CurrentValue);
            $this->pelanggaran_disiplin->PlaceHolder = RemoveHtml($this->pelanggaran_disiplin->caption());
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // inspeksi_kasus
            $this->inspeksi_kasus->setDbValueDef($rsnew, $this->inspeksi_kasus->CurrentValue, null, $this->inspeksi_kasus->ReadOnly);

            // pelanggaran_disiplin
            $this->pelanggaran_disiplin->setDbValueDef($rsnew, $this->pelanggaran_disiplin->CurrentValue, null, $this->pelanggaran_disiplin->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // inspeksi_kasus
        $this->inspeksi_kasus->setDbValueDef($rsnew, $this->inspeksi_kasus->CurrentValue, null, false);

        // pelanggaran_disiplin
        $this->pelanggaran_disiplin->setDbValueDef($rsnew, $this->pelanggaran_disiplin->CurrentValue, null, false);
