 ...
 * Page class
 */
class HukumanDisiplinList extends HukumanDisiplin
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HukumanDisiplinList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fhukuman_disiplinlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "HukumanDisiplinDelete";
        $this->MultiUpdateUrl = "HukumanDisiplinUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fhukuman_disiplinlistsrch";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fhukuman_disiplinlistsrch", $filters);
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->surat_keputusan, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->pernah_dijatuhi_hukuman); // pernah_dijatuhi_hukuman
            $this->updateSort($this->jenis_hukuman); // jenis_hukuman
            $this->updateSort($this->hukuman); // hukuman
            $this->updateSort($this->pasal); // pasal
            $this->updateSort($this->surat_keputusan); // surat_keputusan
            $this->updateSort($this->sk_nomor); // sk_nomor
            $this->updateSort($this->tanggal_sk); // tanggal_sk
            $this->updateSort($this->status_hukuman); // status_hukuman
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fhukuman_disiplinlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fhukuman_disiplinlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fhukuman_disiplinlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fhukuman_disiplinlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
