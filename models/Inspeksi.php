...
/**
 * Table class for inspeksi
 */
class Inspeksi extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'inspeksi';
        $this->TableName = 'inspeksi';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`inspeksi`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('inspeksi', 'inspeksi', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // pid_request_skk
        $this->pid_request_skk = new DbField('inspeksi', 'inspeksi', 'x_pid_request_skk', 'pid_request_skk', '`pid_request_skk`', '`pid_request_skk`', 20, 20, -1, false, '`pid_request_skk`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // inspeksi_kasus
        $this->inspeksi_kasus = new DbField('inspeksi', 'inspeksi', 'x_inspeksi_kasus', 'inspeksi_kasus', '`inspeksi_kasus`', '`inspeksi_kasus`', 202, 5, -1, false, '`inspeksi_kasus`', false, false, false, 'FORMATTED TEXT', 'RADIO');
        $this->inspeksi_kasus->Required = true; // Required field
        $this->inspeksi_kasus->Sortable = true; // Allow sort
        switch ($CurrentLanguage) {
            case "en":
                $this->inspeksi_kasus->Lookup = new Lookup('inspeksi_kasus', 'inspeksi', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->inspeksi_kasus->Lookup = new Lookup('inspeksi_kasus', 'inspeksi', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        $this->pelanggaran_disiplin = new DbField('inspeksi', 'inspeksi', 'x_pelanggaran_disiplin', 'pelanggaran_disiplin', '`pelanggaran_disiplin`', '`pelanggaran_disiplin`', 201, 65535, -1, false, '`pelanggaran_disiplin`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`inspeksi`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("InspeksiList");
    }
    ...
    {
        global $Language;
        if ($pageName == "InspeksiView") {
            return $Language->phrase("View");
        } elseif ($pageName == "InspeksiEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "InspeksiAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("InspeksiView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("InspeksiView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "InspeksiAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "InspeksiAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("InspeksiEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InspeksiAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("InspeksiDelete", $this->getUrlParm());
    }
