 ...
 * Page class
 */
class VKirimSkkEdit extends VKirimSkk
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_kirim_skk';

    // Page object name
    public $PageObjName = "VKirimSkkEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (v_kirim_skk)
        if (!isset($GLOBALS["v_kirim_skk"]) || get_class($GLOBALS["v_kirim_skk"]) == PROJECT_NAMESPACE . "v_kirim_skk") {
            $GLOBALS["v_kirim_skk"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_kirim_skk');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_kirim_skk"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->sk_bandng_nomor->setVisibility();
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("VKirimSkkList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "VKirimSkkList") {
    ...
    protected function getUploadFiles()
    {
        ...
        $this->hasil_acc->Upload->Index = $CurrentForm->Index;
        $this->hasil_acc->Upload->uploadFile();
        $this->hasil_acc->CurrentValue = $this->hasil_acc->Upload->FileName;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'sk_bandng_nomor' first before field var 'x_sk_bandng_nomor'
        $val = $CurrentForm->hasValue("sk_bandng_nomor") ? $CurrentForm->getValue("sk_bandng_nomor") : $CurrentForm->getValue("x_sk_bandng_nomor");
        if (!$this->sk_bandng_nomor->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->sk_bandng_nomor->Visible = false; // Disable update for API request
            } else {
                $this->sk_bandng_nomor->setFormValue($val);
            }
        }
        ...
        if (!$this->tgl_sk_banding->IsDetailKey) {
            ...
            $this->tgl_sk_banding->CurrentValue = UnFormatDateTime($this->tgl_sk_banding->CurrentValue, 0);
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->sk_bandng_nomor->CurrentValue = $this->sk_bandng_nomor->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->isShow()) {
                RenderUploadField($this->hasil_acc);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->sk_bandng_nomor->Required) {
            if (!$this->sk_bandng_nomor->IsDetailKey && EmptyValue($this->sk_bandng_nomor->FormValue)) {
                $this->sk_bandng_nomor->addErrorMessage(str_replace("%s", $this->sk_bandng_nomor->caption(), $this->sk_bandng_nomor->RequiredErrorMessage));
            }
        }
        ...
        if ($this->mengajukan_keberatan_banding->Required) {
            if ($this->mengajukan_keberatan_banding->FormValue == "") {
        ...
        if ($this->inspeksi_kasus->Required) {
            if ($this->inspeksi_kasus->FormValue == "") {
        ...
        if ($this->hasil_acc->Required) {
            if ($this->hasil_acc->Upload->FileName == "" && !$this->hasil_acc->Upload->KeepFile) {
                $this->hasil_acc->addErrorMessage(str_replace("%s", $this->hasil_acc->caption(), $this->hasil_acc->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // hasil_acc
            if ($this->hasil_acc->Visible && !$this->hasil_acc->ReadOnly && !$this->hasil_acc->Upload->KeepFile) {
                $this->hasil_acc->Upload->DbValue = $rsold['hasil_acc']; // Get original value
                if ($this->hasil_acc->Upload->FileName == "") {
                    $rsnew['hasil_acc'] = null;
                } else {
                    $rsnew['hasil_acc'] = $this->hasil_acc->Upload->FileName;
                }
            }

            // status
            $this->status->setDbValueDef($rsnew, $this->status->CurrentValue, null, $this->status->ReadOnly);
            if ($this->hasil_acc->Visible && !$this->hasil_acc->Upload->KeepFile) {
                $oldFiles = EmptyValue($this->hasil_acc->Upload->DbValue) ? [] : [$this->hasil_acc->htmlDecode($this->hasil_acc->Upload->DbValue)];
                if (!EmptyValue($this->hasil_acc->Upload->FileName)) {
                    $newFiles = [$this->hasil_acc->Upload->FileName];
                    $NewFileCount = count($newFiles);
                    for ($i = 0; $i < $NewFileCount; $i++) {
                        if ($newFiles[$i] != "") {
                            $file = $newFiles[$i];
                            $tempPath = UploadTempPath($this->hasil_acc, $this->hasil_acc->Upload->Index);
                            if (file_exists($tempPath . $file)) {
                                ...
                                $file1 = UniqueFilename($this->hasil_acc->physicalUploadPath(), $file); // Get new file name
                                if ($file1 != $file) { // Rename temp file
                                    while (file_exists($tempPath . $file1) || file_exists($this->hasil_acc->physicalUploadPath() . $file1)) { // Make sure no file name clash
                                        $file1 = UniqueFilename([$this->hasil_acc->physicalUploadPath(), $tempPath], $file1, true); // Use indexed name
                                    }
                    ...
                    $this->hasil_acc->Upload->DbValue = empty($oldFiles) ? "" : implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $oldFiles);
                    $this->hasil_acc->Upload->FileName = implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $newFiles);
                    $this->hasil_acc->setDbValueDef($rsnew, $this->hasil_acc->Upload->FileName, null, $this->hasil_acc->ReadOnly);
                }
            }
            ...
            if ($updateRow) {
                ...
                if ($editRow) {
                    if ($this->hasil_acc->Visible && !$this->hasil_acc->Upload->KeepFile) {
                        ...
                        if (!EmptyValue($this->hasil_acc->Upload->FileName)) {
                            $newFiles = [$this->hasil_acc->Upload->FileName];
                            $newFiles2 = [$this->hasil_acc->htmlDecode($rsnew['hasil_acc'])];
                            $newFileCount = count($newFiles);
                            for ($i = 0; $i < $newFileCount; $i++) {
                                if ($newFiles[$i] != "") {
                                    $file = UploadTempPath($this->hasil_acc, $this->hasil_acc->Upload->Index) . $newFiles[$i];
                                    if (file_exists($file)) {
                                        if (@$newFiles2[$i] != "") { // Use correct file name
                                            $newFiles[$i] = $newFiles2[$i];
                                        }
                                        if (!$this->hasil_acc->Upload->SaveToFile($newFiles[$i], true, $i)) { // Just replace
        ...
        if ($editRow) {
            // hasil_acc
            CleanUploadTempPath($this->hasil_acc, $this->hasil_acc->Upload->Index);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("VKirimSkkList"), "", $this->TableVar, true);
        $pageId = "edit";
