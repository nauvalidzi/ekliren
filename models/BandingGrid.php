 ...
 * Page class
 */
class BandingGrid extends Banding
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'banding';

    // Page object name
    public $PageObjName = "BandingGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fbandinggrid";
    ...
    public function __construct()
    {
        ...
        // Table object (banding)
        if (!isset($GLOBALS["banding"]) || get_class($GLOBALS["banding"]) == PROJECT_NAMESPACE . "banding") {
            $GLOBALS["banding"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "BandingAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'banding');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("banding"));
                $doc->Text = @$content;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_mengajukan_keberatan_banding") && $CurrentForm->hasValue("o_mengajukan_keberatan_banding") && $this->mengajukan_keberatan_banding->CurrentValue != $this->mengajukan_keberatan_banding->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_sk_banding_nomor") && $CurrentForm->hasValue("o_sk_banding_nomor") && $this->sk_banding_nomor->CurrentValue != $this->sk_banding_nomor->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tgl_sk_banding") && $CurrentForm->hasValue("o_tgl_sk_banding") && $this->tgl_sk_banding->CurrentValue != $this->tgl_sk_banding->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->mengajukan_keberatan_banding->clearErrorMessage();
        $this->sk_banding_nomor->clearErrorMessage();
        $this->tgl_sk_banding->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->mengajukan_keberatan_banding->CurrentValue = 'Tidak';
        $this->mengajukan_keberatan_banding->OldValue = $this->mengajukan_keberatan_banding->CurrentValue;
        $this->sk_banding_nomor->CurrentValue = null;
        $this->sk_banding_nomor->OldValue = $this->sk_banding_nomor->CurrentValue;
        $this->tgl_sk_banding->CurrentValue = null;
        $this->tgl_sk_banding->OldValue = $this->tgl_sk_banding->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_mengajukan_keberatan_banding")) {
            $this->mengajukan_keberatan_banding->setOldValue($CurrentForm->getValue("o_mengajukan_keberatan_banding"));
        }
        ...
        if ($CurrentForm->hasValue("o_sk_banding_nomor")) {
            $this->sk_banding_nomor->setOldValue($CurrentForm->getValue("o_sk_banding_nomor"));
        }
        ...
        if ($CurrentForm->hasValue("o_tgl_sk_banding")) {
            $this->tgl_sk_banding->setOldValue($CurrentForm->getValue("o_tgl_sk_banding"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['mengajukan_keberatan_banding'] = $this->mengajukan_keberatan_banding->CurrentValue;
        $row['sk_banding_nomor'] = $this->sk_banding_nomor->CurrentValue;
        $row['tgl_sk_banding'] = $this->tgl_sk_banding->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->mengajukan_keberatan_banding->EditValue = $this->mengajukan_keberatan_banding->options(false);
            $this->mengajukan_keberatan_banding->PlaceHolder = RemoveHtml($this->mengajukan_keberatan_banding->caption());

            // sk_banding_nomor
            $this->sk_banding_nomor->EditAttrs["class"] = "form-control";
            $this->sk_banding_nomor->EditCustomAttributes = "";
            if (!$this->sk_banding_nomor->Raw) {
                $this->sk_banding_nomor->CurrentValue = HtmlDecode($this->sk_banding_nomor->CurrentValue);
            }
            $this->sk_banding_nomor->EditValue = HtmlEncode($this->sk_banding_nomor->CurrentValue);
            $this->sk_banding_nomor->PlaceHolder = RemoveHtml($this->sk_banding_nomor->caption());

            // tgl_sk_banding
            $this->tgl_sk_banding->EditAttrs["class"] = "form-control";
            $this->tgl_sk_banding->EditCustomAttributes = "";
            $this->tgl_sk_banding->EditValue = HtmlEncode(FormatDateTime($this->tgl_sk_banding->CurrentValue, 8));
            $this->tgl_sk_banding->PlaceHolder = RemoveHtml($this->tgl_sk_banding->caption());
    ...
    protected function validateForm()
    {
        ...
        if (!CheckDate($this->tgl_sk_banding->FormValue)) {
            $this->tgl_sk_banding->addErrorMessage($this->tgl_sk_banding->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // mengajukan_keberatan_banding
            $this->mengajukan_keberatan_banding->setDbValueDef($rsnew, $this->mengajukan_keberatan_banding->CurrentValue, null, $this->mengajukan_keberatan_banding->ReadOnly);

            // sk_banding_nomor
            $this->sk_banding_nomor->setDbValueDef($rsnew, $this->sk_banding_nomor->CurrentValue, null, $this->sk_banding_nomor->ReadOnly);

            // tgl_sk_banding
            $this->tgl_sk_banding->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_sk_banding->CurrentValue, 0), null, $this->tgl_sk_banding->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // mengajukan_keberatan_banding
        $this->mengajukan_keberatan_banding->setDbValueDef($rsnew, $this->mengajukan_keberatan_banding->CurrentValue, null, false);

        // sk_banding_nomor
        $this->sk_banding_nomor->setDbValueDef($rsnew, $this->sk_banding_nomor->CurrentValue, null, false);

        // tgl_sk_banding
        $this->tgl_sk_banding->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_sk_banding->CurrentValue, 0), null, false);
