 ...
 * Page class
 */
class HukumanDisiplinGrid extends HukumanDisiplin
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HukumanDisiplinGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fhukuman_disiplingrid";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "HukumanDisiplinAdd";
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_pernah_dijatuhi_hukuman") && $CurrentForm->hasValue("o_pernah_dijatuhi_hukuman") && $this->pernah_dijatuhi_hukuman->CurrentValue != $this->pernah_dijatuhi_hukuman->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_jenis_hukuman") && $CurrentForm->hasValue("o_jenis_hukuman") && $this->jenis_hukuman->CurrentValue != $this->jenis_hukuman->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_hukuman") && $CurrentForm->hasValue("o_hukuman") && $this->hukuman->CurrentValue != $this->hukuman->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_pasal") && $CurrentForm->hasValue("o_pasal") && $this->pasal->CurrentValue != $this->pasal->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_surat_keputusan") && $CurrentForm->hasValue("o_surat_keputusan") && $this->surat_keputusan->CurrentValue != $this->surat_keputusan->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_sk_nomor") && $CurrentForm->hasValue("o_sk_nomor") && $this->sk_nomor->CurrentValue != $this->sk_nomor->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tanggal_sk") && $CurrentForm->hasValue("o_tanggal_sk") && $this->tanggal_sk->CurrentValue != $this->tanggal_sk->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_status_hukuman") && $CurrentForm->hasValue("o_status_hukuman") && $this->status_hukuman->CurrentValue != $this->status_hukuman->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->pernah_dijatuhi_hukuman->clearErrorMessage();
        $this->jenis_hukuman->clearErrorMessage();
        $this->hukuman->clearErrorMessage();
        $this->pasal->clearErrorMessage();
        $this->surat_keputusan->clearErrorMessage();
        $this->sk_nomor->clearErrorMessage();
        $this->tanggal_sk->clearErrorMessage();
        $this->status_hukuman->clearErrorMessage();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_pernah_dijatuhi_hukuman")) {
            $this->pernah_dijatuhi_hukuman->setOldValue($CurrentForm->getValue("o_pernah_dijatuhi_hukuman"));
        }
        ...
        if ($CurrentForm->hasValue("o_jenis_hukuman")) {
            $this->jenis_hukuman->setOldValue($CurrentForm->getValue("o_jenis_hukuman"));
        }
        ...
        if ($CurrentForm->hasValue("o_hukuman")) {
            $this->hukuman->setOldValue($CurrentForm->getValue("o_hukuman"));
        }
        ...
        if ($CurrentForm->hasValue("o_pasal")) {
            $this->pasal->setOldValue($CurrentForm->getValue("o_pasal"));
        }
        ...
        if ($CurrentForm->hasValue("o_surat_keputusan")) {
            $this->surat_keputusan->setOldValue($CurrentForm->getValue("o_surat_keputusan"));
        }
        ...
        if ($CurrentForm->hasValue("o_sk_nomor")) {
            $this->sk_nomor->setOldValue($CurrentForm->getValue("o_sk_nomor"));
        }
        ...
        if ($CurrentForm->hasValue("o_tanggal_sk")) {
            $this->tanggal_sk->setOldValue($CurrentForm->getValue("o_tanggal_sk"));
        }
        ...
        if ($CurrentForm->hasValue("o_status_hukuman")) {
            $this->status_hukuman->setOldValue($CurrentForm->getValue("o_status_hukuman"));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // pernah_dijatuhi_hukuman
            $this->pernah_dijatuhi_hukuman->setDbValueDef($rsnew, $this->pernah_dijatuhi_hukuman->CurrentValue, null, $this->pernah_dijatuhi_hukuman->ReadOnly);

            // jenis_hukuman
            $this->jenis_hukuman->setDbValueDef($rsnew, $this->jenis_hukuman->CurrentValue, null, $this->jenis_hukuman->ReadOnly);

            // hukuman
            $this->hukuman->setDbValueDef($rsnew, $this->hukuman->CurrentValue, null, $this->hukuman->ReadOnly);

            // pasal
            $this->pasal->setDbValueDef($rsnew, $this->pasal->CurrentValue, null, $this->pasal->ReadOnly);

            // surat_keputusan
            $this->surat_keputusan->setDbValueDef($rsnew, $this->surat_keputusan->CurrentValue, null, $this->surat_keputusan->ReadOnly);

            // sk_nomor
            $this->sk_nomor->setDbValueDef($rsnew, $this->sk_nomor->CurrentValue, null, $this->sk_nomor->ReadOnly);

            // tanggal_sk
            $this->tanggal_sk->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_sk->CurrentValue, 0), null, $this->tanggal_sk->ReadOnly);

            // status_hukuman
            $this->status_hukuman->setDbValueDef($rsnew, $this->status_hukuman->CurrentValue, null, $this->status_hukuman->ReadOnly);
