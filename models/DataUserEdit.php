 ...
 * Page class
 */
class DataUserEdit extends DataUser
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DataUserEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_user") ?? Key(0) ?? Route(2)) !== null) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // email_satker
            $this->email_satker->setDbValueDef($rsnew, $this->email_satker->CurrentValue, null, $this->email_satker->ReadOnly);

            // hak_akses
            if ($Security->canAdmin()) { // System admin
                $this->hak_akses->setDbValueDef($rsnew, $this->hak_akses->CurrentValue, "", $this->hak_akses->ReadOnly);
            }
