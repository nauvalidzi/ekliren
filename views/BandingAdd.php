...
loadjs.ready("head", function () {
    ...
    fbandingadd = currentForm = new ew.Form("fbandingadd", "add");
    ...
    // Form_CustomValidate
    fbandingadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbandingadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fbandingadd.lists.mengajukan_keberatan_banding = <?= $Page->mengajukan_keberatan_banding->toClientList($Page) ?>;
    loadjs.done("fbandingadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fbandingadd" id="fbandingadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->pid_request_skk->Visible) { // pid_request_skk ?>
    <div id="r_pid_request_skk" class="form-group row">
        <label id="elh_banding_pid_request_skk" for="x_pid_request_skk" class="<?= $Page->LeftColumnClass ?>"><?= $Page->pid_request_skk->caption() ?><?= $Page->pid_request_skk->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<script>
loadjs.ready(["fbandingadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fbandingadd", "x_tgl_sk_banding", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
