...
// Page object
$HukumanDisiplinAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fhukuman_disiplinadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    fhukuman_disiplinadd = currentForm = new ew.Form("fhukuman_disiplinadd", "add");
    ...
    fhukuman_disiplinadd.addFields([
    ...
    // Validate form
    fhukuman_disiplinadd.validate = function () {
    ...
    // Form_CustomValidate
    fhukuman_disiplinadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fhukuman_disiplinadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fhukuman_disiplinadd.lists.pernah_dijatuhi_hukuman = <?= $Page->pernah_dijatuhi_hukuman->toClientList($Page) ?>;
    fhukuman_disiplinadd.lists.jenis_hukuman = <?= $Page->jenis_hukuman->toClientList($Page) ?>;
    fhukuman_disiplinadd.lists.surat_keputusan = <?= $Page->surat_keputusan->toClientList($Page) ?>;
    fhukuman_disiplinadd.lists.status_hukuman = <?= $Page->status_hukuman->toClientList($Page) ?>;
    loadjs.done("fhukuman_disiplinadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fhukuman_disiplinadd" id="fhukuman_disiplinadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->pid_request_skk->Visible) { // pid_request_skk ?>
    <div id="r_pid_request_skk" class="form-group row">
        <label id="elh_hukuman_disiplin_pid_request_skk" for="x_pid_request_skk" class="<?= $Page->LeftColumnClass ?>"><?= $Page->pid_request_skk->caption() ?><?= $Page->pid_request_skk->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<script>
loadjs.ready(["fhukuman_disiplinadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fhukuman_disiplinadd", "x_tanggal_sk", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
